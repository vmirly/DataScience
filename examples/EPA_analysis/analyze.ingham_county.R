...
d.ingham.motor <- d.ingham[d.ingham$SCC %in% scc.motor,]

print(str(d.bm.motor))
print(str(d.la.motor))
print(str(d.ingham.motor))
...
emis_year.ingham <- tapply(d.ingham.motor$Emissions, factor(d.ingham.motor$year), sum)
print(emis_year.bm)
print(emis_year.la)
print(emis_year.ingham)
...
lines(names(emis_year.la), emis_year.ingham, lwd=5,
        col="darkgreen")
points(names(emis_year.la), emis_year.ingham,
        col="darkgreen", pch=17, cex=1.6)
